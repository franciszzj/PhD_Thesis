...
Building on Chapter~3, this chapter addresses a second obstacle to controllable relation prediction: rare and ambiguous relations are often difficult to resolve from visual evidence alone.
The setting remains closed-set PSG, but the goal now is to widen the control interface with language priors. As Section~\ref{sec:unifying_argument} notes, supplying those priors is not sufficient on its own, so this chapter also has to teach the model when to trust them.
