...
Inspired by~\citep{darcet2023vision}, we average the attention maps across all heads before computing the \leffa{} loss.
...
\section{Additional Notes on Limitations}
Sec.~\ref{leffa:sec:limitations} discusses the limitations of \leffa{} in the main chapter. We note here only that three of the issues raised there concern off-the-shelf components: the reliance on garment segmentation for appearance control, the $8\times$ compression of the latent encoder, and the multi-stage training schedule. These issues are not unique to our method and are shared with other latent-diffusion approaches to this task.
