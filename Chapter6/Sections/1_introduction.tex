...
Chapters~3--5 widened what a user can control in a scene representation. The question now changes. A user of a reference-conditioned generator already has an explicit control signal, namely the reference image, so the issue is no longer what can be specified but whether the model honors what has been specified.
The specific question in this chapter is how a diffusion model can be made to preserve fine-grained reference details in a single generated image, before Chapter~7 extends the same problem to videos.
