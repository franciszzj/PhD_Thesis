...
Chapter~6 opens the visual generation part of the thesis.
Where Chapters~3--5 represent task-relevant structure as pixel-grounded entities and relations, Chapters~6--7 represent it as correspondence between a generated target and its references. These structures are not interchangeable, but they play the same functional role: each specifies the relationships that learning must preserve to satisfy the task.
This chapter focuses on spatial correspondence. Its specific question is how a diffusion model can preserve fine-grained reference details in a single generated image, before Chapter~7 extends the requirement to temporally consistent identity in videos.
...
Within the thesis, this chapter establishes the image-level correspondence mechanism for Part~II.
Chapter~7 builds on the same reference-preservation perspective and studies how to scale it to reference-to-video generation.
