...
Given an image $I$, we utilize the pretrained open-set object segmenter (\eg, OpenSeeD~\citep{zhang2023simple}) to predict the objects $O$ within the image and the whole-image visual feature $F_I \in \mathbb{R}^{h \times w \times D}$. 
Here, $h$ and $w$ represent the height and width of $F_I$, and $D$ denotes the feature dimension.
The segmenter has a similar architecture to Mask2Former~\citep{cheng2022masked} includeing a pixel decoder.
...
Similar to the input patchify layer of vision transformer (ViT)~\citep{dosovitskiy2020image}, we utilize a single convolution layer to transform the extracted $F_I$ into a sequence of visual tokens $F_{Iseq} \in \mathbb{R}^{L \times D}$, where $L$ is the number of patches and $D$ is the feature dimension.
...
A common extraction method involves mask pooling~\citep{dai2015convolutional}, which extracts features for the target subject-object pair, treating each area on the subject-object pair equally.
...
Furthermore, inspired by \citep{liu2023improved}, we design an instruction to assist the this learnable query in understanding its purpose for extracting subject-object pair features. 
...
Inspired by \citep{yue2023object, yu2023towards}, we first design a \textit{generation instruction} to perform open-set relation prediction in an autoregressive manner.
...
For the generation instruction, we follow the instruction design used in open-set object recognition~\citep{yu2023towards}, utilizing ``What are the relations between $c_i$ and $c_j$?''.
