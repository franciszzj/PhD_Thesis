...
Chapters~3 and~4 improved how a model learns relational structure within a fixed label space, first across the head and tail of the distribution and then through language-derived knowledge. The remaining limitation is the representation of the relation itself: classification confines every prediction to a predefined predicate vocabulary.
The goal here is therefore to extend PSG toward open-set relation prediction while retaining pixel-level grounding. OpenPSG replaces fixed-vocabulary predicate classification with pair-conditioned autoregressive decoding and filters irrelevant pairs before generation, thereby changing both the structural representation and the decoding procedure.
...
Within the thesis, OpenPSG completes the progression from long-tail closed-set PSG (Chapter~3), through language-assisted closed-set PSG (Chapter~4), toward open-world grounded relation prediction.
The next chapter starts Part~II and studies structure-aligned learning for reference-conditioned visual generation.
