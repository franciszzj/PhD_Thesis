...
The challenge is not only to preserve identity from references, but also to maintain that identity over time, and to do so without relying on costly reference image--video--text triplets. As in Chapter~6, supplying the reference is not enough. A model trained on video can satisfy its objective by copying neighboring frames, so the intended use of the reference has to be enforced by the training task itself.
