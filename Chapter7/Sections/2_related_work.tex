...
\label{saber:sec:post_publication}
Saber is the most recent work in this thesis, so the literature after it is correspondingly thin. Two directions have nonetheless emerged that clarify the position of this chapter, and they point in opposite ways.

\noindent \textbf{Better data rather than no data.}
Saber assumes that curated reference-video-text triplets are a bottleneck to be removed. An alternative response is to build the triplets better. Vera~\citep{xu2026vera} constructs a million-pair identity-aligned image-video dataset through person-level cross-clip retrieval, so that the reference and the target come from \emph{different} clips of the same person, and combines this with identity-focal masked supervision and reference-aware layer-wise attention. This directly answers the concern raised in Sec.~\ref{saber:sec:limitations}. Cross-clip retrieval decorrelates the reference from the target photometrically as well as geometrically, and therefore removes the shared illumination and shared camera statistics that our spatial augmentation leaves intact. Vera also targets excessive reference replication, which is the copy-paste shortcut, at the architectural level.

Vera therefore addresses the weakness of the Saber training signal, at the cost of reinstating the data-construction pipeline that Saber exists to avoid. The comparison is instructive rather than damaging. It suggests that the two properties we care about, subject breadth and identity fidelity, are currently traded against one another, and that this trade follows from how references are obtained rather than from anything in the model. The random masks of Saber give breadth without fidelity on faces, whereas retrieval-based pairing gives fidelity on people at the cost of curation expense and human-centric bias. A construction that decorrelates references photometrically \emph{without} requiring identity annotation would obtain both, and no published method does this.

\noindent \textbf{What the reference is for.}
A second line of work questions an assumption that Saber shares with its baselines, namely that the goal is always maximal fidelity to the reference. DomainShuttle~\citep{chen2026domainshuttle} separates in-domain generation, where reference features should be retained as fully as possible, from cross-domain generation, where intrinsic subject identity must persist while subject-irrelevant properties vary according to the prompt. It argues that methods optimized for the first case are correspondingly inflexible in the second. Saber is a fidelity-maximizing method, and this framing exposes a limitation that Sec.~\ref{saber:sec:limitations} does not state. Masked reconstruction teaches the model to reproduce what it sees in the reference, with no mechanism for separating reference properties that are intrinsic to the subject from properties that are incidental to the frame it was cropped from. When a prompt asks for a change to an incidental property, the training signal works against the prompt.

\noindent \textbf{Assessment.}
The central claim of this chapter, that competitive R2V can be trained from ordinary video-text pairs, is not contradicted by this work and is arguably reinforced by it, since the alternative still requires expensive curation. The claim that should be held more loosely is that masked training fully substitutes for genuine reference-target pairs. The results of Vera suggest that it is a good approximation whose residual gap lies exactly where our own evaluation shows Saber to be weakest. On the evidence of DomainShuttle, separating intrinsic from incidental reference properties is the more fundamental of the two open problems.
