...
\begin{wrapfigure}{r}{0.6\textwidth}
    ...
    \includegraphics[width=1.0\linewidth]{Chapter7/Figures/mask_frames.pdf}
    \vspace{-6mm}
    \caption{
    \textbf{Masked reference generation.}
...
\paragraph{Mask Generator.}
We randomly select one mask type from predefined shape categories (\eg, ellipse, Fourier blob, convex/concave polygon, \etc) to generate a binary mask $\mathbf{M} \in \{ 0, 1 \}^{H \times W}$ with a target foreground area ratio $r \in [r_\text{min}, r_\text{max}]$.
...
\begin{wrapfigure}{r}{0.6\textwidth}
    \centering
    % \fbox{\rule{0pt}{2in} \rule{1.0\linewidth}{0pt}}
    \includegraphics[width=1.0\linewidth]{Chapter7/Figures/model_design.pdf}
    \vspace{-6mm}
    \caption{
    \textbf{Model design overview.}
