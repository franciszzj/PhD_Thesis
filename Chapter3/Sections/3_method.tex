...
Fig.~\ref{hilo:fig:baseline} gives an overview of the complete framework, including the relation swapping module, the two parallel decoders, and the consistency losses that align them. We first describe the baseline on which the framework is built.
High-quality panoptic segmentation is crucial for achieving good PSG performance.
We build our method upon recent advances in DETR-based panoptic segmentation, namely Mask2Former \citep{cheng2022masked}.
...
The original query in Mask2Former is to predict the object. In order to predict both subject, object and their relation, we develop the triplet queries $\mathcal{Q}^{t}$ inspired by \citep{yang2022panoptic} into our baseline.
...
\citep{cheng2022masked} proposes a transformer decoder variant with masked attention, which extracts spatial features by adding the predicted mask of the object from the previous decoder layer's mask prediction.
...
We adopt the same losses as PSGTR~\citep{yang2022panoptic}, including cross-entropy loss $\mathcal{L}_{so\_cls}$ for object classification of subject-object pairs and a combination $\mathcal{L}_{so\_mask}$ of focal loss and dice loss to jointly supervise mask learning.
...
%
This is reflected in the PSG dataset~\citep{yang2022panoptic} where many subject-object pairs are annotated with multiple relation labels. These relation labels are of different frequencies in the dataset. In this section we introduce the HiLo relation generation module to prepare two sets of training data, biasing towards high and low-frequency relations respectively.

\noindent \textbf{Relation Augmentation}. 
Similar to~\citep{goel2022not}, we observe that there are many missing relation annotations in the PSG dataset~\citep{yang2022panoptic}.
To add the missing relation annotations, we design a relation augmentation scheme that is inspired by IETrans \citep{zhang2022fine}, which converts high-frequency relations to low-frequency relations and adds a relation to a subject-object pair that has no relation.
%
...
The H-L and L-H decoders favor low-frequency relation prediction and high-frequency relation prediction, respectively. 
