...
Chapter~3 opens the visual understanding part of the thesis. In the terms of Section~\ref{sec:unifying_argument}, relation prediction is the interface through which a scene can be controlled, since a relation is the smallest unit of a scene description that a user can ask about or verify. This chapter addresses the first obstacle to that control: a model given a vocabulary of $56$ predicates has no incentive to use the rare ones, and predicts frequent predicates almost exclusively.
The goal of this chapter is therefore to establish a strong closed-set PSG formulation that recovers both common and rare predicates, before the later chapters add language priors and open-set capability.
